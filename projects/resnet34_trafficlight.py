...
model = dict(
    ...
    head=dict(
        type='TrafficSubLightMultiClsHead',
        shape_classes=8,
        color_classes=4,
        toward_classes=2,
        character_classes=6,
...
# dataset settings
dataset_type = 'TrafficSubLightClsDataset'
dataset_root = '/share/zbh/Datasets/'
img_norm_cfg = dict(
    mean=[0, 0, 0], std=[255., 255., 255.], to_rgb=True)
train_pipeline = [
    dict(type='LoadImageFromFile'),
    #dict(type='SimDet', u=0, std=0.1),
    dict(type='GussCropByBbox', save_crop_result=False),
...
data = dict(
    ...
    train=dict(
        type=dataset_type,
        ann_file=dataset_root + "2022_Q1_icu30_new_crop_correct_zs_train.json",
        # ann_file=dataset_root + "traffic_light_test.json",
        # ann_file=dataset_root + "traffic_light_debug.json",
        data_prefix=dataset_root+'2022_Q1_icu30_new_crop_correct/',
        pipeline=train_pipeline),
    val=dict(
        type=dataset_type,
        ann_file=dataset_root + "2022_Q1_icu30_new_crop_correct_zs_test.json",
        # ann_file=dataset_root + "traffic_light_debug.json",
        data_prefix=dataset_root+'2022_Q1_icu30_new_crop_correct/',
