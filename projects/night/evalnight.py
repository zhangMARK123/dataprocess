...
def plotcrop(imgpath,labelpath,wpath):
    ...
    for obj in labels["objects"]:
        ...
        if obj["lightboxshape_head"]==str(True) and obj["label_shape_pred"]!=obj["label_shape_gt"]:
            showflag=True
        # if obj["toward_head"]==str(True) and obj["label_toward_pred"]!=obj["label_toward_gt"]:
        #     showflag=True
...
def evalresult(labelpath,wpath):
    ...
    for result_output in labels["objects"]:
        # if result_output["ext_occlusion"] == 1 or result_output["truncation"] == 1:
        #         continue    
            #筛选正方向灯箱     
        # if min(result_output["bbox"][2], result_output["bbox"][3])<=0:
        #     continue
        # if max(result_output["bbox"][2], result_output["bbox"][3]) / min(result_output["bbox"][2], result_output["bbox"][3]) < 2.5:
        #     continue
        if result_output["label_toward_gt"]!='0':
            result_output["lightboxshape_head"]=False
            result_output["lightboxcolor_head"]=False

        if result_output["label_color_gt"] in ['0','1','2','3'] and result_output["label_shape_gt"] in ['4','5']:
            result_output["lightboxshape_head"]=False
    ...
    "label_toward_pred":towardpred,
    "label_toward_gt":towardgt,'toward_head':towardhead,"imgname":name})
    ####颜色
...
labelpath="../../work_dirs/neightlight/12_result.json"
wpath="../work_dirs/neightlight/12_result_eval.json"
evalresult(labelpath,wpath)


imgpath="/disk3/zbh/Datasets/2022_Q1_night/"
wimg="../../work_dirs/neightlight/fliter2/testshape"
