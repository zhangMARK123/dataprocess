...
model = dict(
    ...
    head=dict(
        type='TrafficSubLightneightMultiClsHead',
        shape_classes=6,
        color_classes=5,
        toward_classes=3,
        in_channels=256,
...
# dataset settings
dataset_type = 'TrafficSubLightnightClsDataset'
dataset_root = '/disk3/zbh/Datasets/'
img_norm_cfg = dict(
    mean=[0, 0, 0], std=[255., 255., 255.], to_rgb=True)
train_pipeline = [
    dict(type='LoadImageFromFile'),
    dict(type='CropByBbox', expand_prob=0.3),
...
test_pipeline = [
    ...
    dict(type='Collect', keys=['img'], meta_keys=('filename', 'ori_filename', 'ori_shape',
                                                  ...
                                                  "lightboxshape_head",
                                                  'toward_orientation', 'toward_head'))
]
data = dict(
    ...
    train=dict(
        type=dataset_type,
        ann_file='/disk3/zbh/Datasets/2022_Q1_icu30_allnight.json',
        data_prefix='/disk3/zbh/Datasets/2022_Q1_night/',
        pipeline=train_pipeline),
        
    val=dict(
        type=dataset_type,
        ann_file=dataset_root + "2022_Q1_icu30_allnight.json",
        data_prefix=dataset_root + '2022_Q1_night/',
...
resume_from = None
workflow = [('train', 1)]
work_dir = "./work_dirs/neightlight/fliter2_sampler"
