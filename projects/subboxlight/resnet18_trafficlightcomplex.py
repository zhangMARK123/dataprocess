...
model = dict(
    ...
    head=dict(
        type='TrafficSubLightsplitMultiClsHead',
        shape_classes=8,
        color_classes=6,
        toward_classes=4,
        character_classes=6,
...
# dataset settings
dataset_type = 'TrafficSplitSubLightClsDataset'
dataset_root = '/disk3/zbh/Datasets/'
img_norm_cfg = dict(
    mean=[0, 0, 0], std=[255., 255., 255.], to_rgb=True)
train_pipeline = [
    dict(type='LoadImageFromFile'),
    dict(type='CropByBbox', expand_prob=0),
    # dict(type='Rotateimg',prob=0),
    ...
    dict(type='ToTensor',
         keys=['color', "color_head", 'shape', "shape_head", 'toward', "toward_head",
               'characteristic', "character_head"]),
    dict(type='Collect',
         keys=['img', 'color', "color_head", 'shape', "shape_head", 'toward',
               "toward_head", 'characteristic', "character_head"])
]
test_pipeline = [
    dict(type='LoadImageFromFile'),
    dict(type='CropByBbox', expand_prob=0.),
    ...
    dict(type='Collect', keys=['img'], meta_keys=('filename', 'ori_filename', 'ori_shape',
                                                  ...
                                                  'img_norm_cfg', 'color', "color_head", 'shape',
                                                  "shape_head",
                                                  'toward', 'toward_head', 'characteristic',
                                                  "character_head"))
]
data = dict(
    ...
    train=dict(
        type=dataset_type,
        ann_file='/disk3/zbh/Datasets/2022_Q1_icu30_train_splitbox.json',
        data_prefix='/disk3/zbh/Datasets/2022_Q1_icu30_crop/',
        pipeline=train_pipeline),      
    val=dict(
        type=dataset_type,
        ann_file=dataset_root + "2022_Q1_icu30_test_splitbox.json",
...
resume_from = None
workflow = [('train', 1)]
work_dir = "./work_dirs/test_splitbox/splitbox1"
