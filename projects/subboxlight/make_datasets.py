...
train_set_json_file =data_root+ "../2022_Q1_icu30_test_splitbox.json"
test_set_json_file =data_root+ "../2022_Q1_icu30_test_splitbox1.json"
debug_set_json_file =data_root+ "../2022_Q1_icu30_debug_splitbox1.json"
img_write_path="/disk3/zbh/Datasets/2022_Q1_icu30_crop/"
card_id_list = []
object_list = []
OBJECT_COUNT = 0
for data_card_id in os.listdir(data_root):
    if data_card_id not in [
        "61e65695910d7200e733b1a0",
        ...
        "61e10a9c910d7200e7b7b55d",
        "6209fb57d3572380d5767ff4",
        "61ee749f761998edc2524071",
        "61f0ad7b761998edc2b70800",
        "6209f9a0d3572380d56f0080",
        "61e6a5cd910d7200e7530be2"
    ]:
        continue
    ...
    for idx, json_file in enumerate(os.listdir(os.path.join(data_root, data_card_id, "labels"))):
        with open(os.path.join(data_root, data_card_id, "labels", json_file)) as f:
            ...
            for object in sample_json["objects"]:
                ...
                object["color_head"] = True
                object["shape_head"] = True
                ...
                index_per_img += 1
                if object["num_sub_lights"]<=1:
                    continue
                if object["num_sub_lights"]!=len(object["splits"])+1:
                    continue
                ...
                split_ori=object["splits"]
                count_split=0
                split_bbox=[]
                for sp in split_ori:
                    if count_split==0:
                        temp=[x1,y1,sp[2]-bbox_ori[0],sp[3]-bbox_ori[1]]
                        
                    else:
                        temp=[split_ori[count_split-1][0]-bbox_ori[0]+x1,split_ori[count_split-1][1]-bbox_ori[1]+y1,sp[2]-split_ori[count_split-1][0],sp[3]-split_ori[count_split-1][1]]
                    count_split+=1
                    # if sp[0]>0 and sp[1]>0 and sp[2]>0 and sp[3]>0:
                    
                    split_bbox.append(temp)
                split_bbox.append([split_ori[-1][0]-bbox_ori[0]+x1,split_ori[-1][1]-bbox_ori[1]+y1,bbox_ori[0]+bbox_ori[2]-split_ori[-1][0],bbox_ori[1]+bbox_ori[3]-split_ori[-1][1]])
                    
                object["bbox"] = [x1, y1, bbox_ori[2], bbox_ori[3]]  # 框在子图的坐标
                object["split_bbox"] = split_bbox
                ...
                if object["toward_orientation"] == 2 or object["toward_orientation"] == 3:
                    ...
                    object["color_head"] = False
                    object["shape_head"] = False
                # 若非通行灯，简单灯head不计入训练，但颜色和形状是否记入训练？？颜色计入训练
                ##此时的颜色如何分辨？
                if object["characteristic"] not in [0,1]:
                    object["color_head"] = False
                    object["shape_head"] = False

        
                n=object["num_sub_lights"]
                for i in range(n):
                    subobj={}
                    subshape=7
                    sub_lights=object["sub_lights"][i]
                    if sub_lights["color"] not in [0,1,2,4] and (object["characteristic"] not in [0,1] or  random.random() < 0.9):
                        continue
                    subobj["color"]=sub_lights["color"]
                    if sub_lights["shape"] == 0:
                        subshape = 0
                    elif sub_lights["shape"] == 1:     # 非机动车
                        subshape = 6
                    elif sub_lights["shape"] == 2:                              
                        if sub_lights["arrow_orientation"]["up_arrow"]==1:
                            subshape=1
                        elif sub_lights["arrow_orientation"]["down_arrow"]==1:
                            subshape=2
                        elif sub_lights["arrow_orientation"]["left_arrow"]==1:
                            subshape=3
                        elif sub_lights["arrow_orientation"]["right_arrow"]==1:
                            subshape=4
                        elif sub_lights["arrow_orientation"]["u_turn"]==1:
                            subshape=5
                    subobj["shape"]=subshape
                    subobj["toward"]=object["toward_orientation"] 
                    subobj["characteristic"]=object["characteristic"]
                    subobj["color_head"]=object["color_head"]
                    subobj["character_head"] = object["character_head"]
                    subobj["toward_head"] = object["toward_head"]
                    subobj["shape_head"] =object["shape_head"]
                    subobj["bbox"]=split_bbox[i]
                    subobj["allbbox"]=object["bbox"]
                    subobj["data_card_id"] = data_card_id
                    
                    subobj["ext_occlusion"]=object["ext_occlusion"]
                    subobj["truncation"]=object["truncation"]
                    OBJECT_COUNT += 1
                    subobj["id"] = '{:0>10d}'.format(OBJECT_COUNT)
                    subobj["img_info"] = {
                    'filename': "images/" + json_file.split(".")[0] + "_" + str(index_per_img-1)+ ".jpg"}
                    
                    
                    object_list.append(subobj)
