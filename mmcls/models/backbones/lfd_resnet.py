...
class FastBlock(BaseModule):
    def __init__(self,
                 ...
                 norm_cfg=None,
                 init_cfg=None):
        super(FastBlock, self).__init__(init_cfg)
        ...
        self._conv1 = nn.Conv2d(in_channels=self._num_input_channel,
                                out_channels=self._num_block_channel,
                                kernel_size=3,
                                stride=self._stride,
                                padding=1,
                                bias=True if self._norm_cfg is None else False)
...
class FasterBlock(BaseModule):
    def __init__(self,
                 ...
                 norm_cfg=None,
                 init_cfg=None):
        super(FasterBlock, self).__init__(init_cfg)
        if downsample is not None:
            assert stride == 2
        if norm_cfg is not None:
            assert norm_cfg['type'] in ['BatchNorm2d', 'GroupNorm']
...
class FastestBlock(BaseModule):
    def __init__(self,
                 ...
                 norm_cfg=None,
                 init_cfg=None):
        super(FastestBlock, self).__init__(init_cfg)
        if downsample is not None:
            assert stride == 2
        if norm_cfg is not None:
            assert norm_cfg['type'] in ['BatchNorm2d', 'GroupNorm']
        ...
        self._conv1 = nn.Conv2d(in_channels=self._num_input_channel,
                                out_channels=self._num_block_channel // 2,
                                kernel_size=3,
                                stride=self._stride,
                                padding=1,
                                bias=True if self._norm_cfg is None else False)
...
class LFDResNet(BaseModule):
    ...
    def __init__(self,
                 ...
                 norm_eval=False,
                 init_cfg=None):
        super(LFDResNet, self).__init__(init_cfg)
    ...
    def _make_stem(self):
        ...
        if self._stem_mode == 'fast':
            stem_layer_list.append(nn.Conv2d(in_channels=self._input_channels,
            ...
            stem_layer_list.append(nn.Conv2d(in_channels=self._stem_channels,
                                             out_channels=self._stem_channels,
                                             kernel_size=1,
                                             stride=1,
                                             padding=0,
            ...
            stem_layer_list.append(nn.Conv2d(in_channels=self._input_channels,
                                             out_channels=self._stem_channels // 2,
                                             kernel_size=3,
                                             stride=2,
                                             padding=1,
                                             bias=True if self._norm_cfg is None else False))
            ...
            stem_layer_list.append(nn.Conv2d(in_channels=self._stem_channels // 2,
                                             out_channels=self._stem_channels,
                                             kernel_size=3,
                                             stride=2,
                                             padding=1,
        ...
        for i, num_blocks in enumerate(self._body_architecture):
            ...
            for j in range(num_blocks):
                ...
                if j == 0:
                    ...
                    downsample_list.append(nn.Conv2d(in_channels=in_channels,
                                                     out_channels=num_stage_channels,
                                                     kernel_size=1,
                                                     stride=2,
                                                     padding=0,
                                                     bias=True if self._norm_cfg is None else False))
