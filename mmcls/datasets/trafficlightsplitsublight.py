...
class TrafficSplitSubLightClsDataset(BaseDataset):
    ...
    def _set_group_flag(self):
        ...
        for i in range(len(self)):
            ...
            if img_info['shape_head'] and img_info['shape'] in [5,6]:
                self.flag[i] = 1
            if img_info['color_head'] and img_info['color'] in [1]:
                self.flag[i] = 1
    ...
    def load_annotations(self):
        ...
        for sample_info in self.samples:
            ...
            if sample_info["color"]==5:
                sample_info["color"]=3
            if random.random()<0.5 and sample_info["color"]==3:
                continue
            if not self.test_mode:
                if sample_info["color"] in [0,1,2,4] and sample_info["shape"]==7:
                    sample_info["shape_head"]=False
                
                ###需要判断下others颜色是否对准确率有影响，若有则舍弃掉
                if sample_info["color"] in [0,1,2,4] and sample_info["shape"]==7 and sample_info["toward"]==1:
                    sample_info["color_head"]=False
                if sample_info["color"]==4:
                    sample_info["color_head"]=False
                if sample_info["shape"]==2:
                    sample_info["shape_head"]=False
                if sample_info["characteristic"]==1 and sample_info["toward"]==1:
                    sample_info["character_head"]=False


            if sample_info["bbox"][2] >=10 and sample_info["bbox"][3] >=10 and sample_info["bbox"][0]>0 and sample_info["bbox"][1]>0:
    ...
    def get_gt_labels(self):
        ...
        color_mask = np.array([data["color_head"] for data in self.data_infos])
        shape_mask = np.array([data["shape_head"] for data in self.data_infos])
        ...
        color_labels = np.array([data["color"] for data in self.data_infos])
        shape_labels = np.array([data["shape"] for data in self.data_infos])
        toward_labels = np.array([data['toward'] for data in self.data_infos])
        ...
        return color_mask, color_labels_selected, shape_mask, shape_labels_selected, toward_mask, toward_labels_selected,character_mask,character_labels_selected
    def get_sampler_labels(self):
        ...
        return color_labels,shape_labels,toward_labels,character_labels
    ...
    def evaluate(self,
        ...
        color_mask, color_labels, shape_mask, shape_labels, toward_mask, toward_labels,character_mask,character_labels= self.get_gt_labels()
        ...
        character_result = results[:, len(self.COLOR_CLASSES)+len(self.SHAPE_CLASSES)+len(self.TOWARD_CLASSES):][character_mask]
        ...
        if True:
            self.saveflag+=1
            savename="/disk3/zs1/mmclassification/work_dirs/test_splitbox/"+str(self.saveflag)+"_result.json"
            ...
            for i,img_meta in enumerate(img_metas): 
                ...
                result_output['allbbox']=img_meta['allbbox']
                result_output['toward_head'] = img_meta['toward_head']
                result_output['character_head'] = img_meta['character_head']
                result_output['lightboxshape_head'] = img_meta['shape_head']
                result_output['lightboxcolor_head'] = img_meta['color_head']
                ...
                result_output["label_toward_gt"] = img_meta['toward']
        ...
        if 'accuracy' in metrics:
            ...
            if isinstance(topk, tuple):
                ...
                eval_results_ = {'color_accuracy': color_acc, 'shape_accuracy': shape_acc, 'toward_accuracy': toward_acc,'character_accuracy':character_acc}
