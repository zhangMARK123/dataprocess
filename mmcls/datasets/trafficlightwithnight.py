...
class TrafficSubLightnightClsDataset(BaseDataset):
    ...
    def _set_group_flag(self):
        ...
        for i in range(len(self)):
            ...
            if img_info['lightboxshape_head'] and img_info['boxshape'] in [2,3]:
                self.flag[i] = 1
            # if img_info['character_head'] and img_info['characteristic'] in [1]:
            #     self.flag[i] = 1
    ...
    def load_annotations(self):
        ...
        for sample_info in self.samples:
            ...
            ###unknow 类数据不需要这么多
            # if random.random()<0.5 and sample_info["boxcolor"]==5:
            #     continue

            ####对数据进行筛选，注意          
            # if not self.test_mode:
            if sample_info['toward_orientation']==2:
                sample_info["toward_head"]=False
            if sample_info['toward_orientation']!=0:
                sample_info["lightboxshape_head"]=False
                sample_info["lightboxcolor_head"]=False

            if sample_info["boxshape"] in [4,5]:
                sample_info["lightboxshape_head"]=False
                # sample_info["lightboxcolor_head"]=False
            if sample_info["boxcolor"] in [3,4]:   
                sample_info["lightboxcolor_head"]=False
            
            ###需要判断下others颜色是否对准确率有影响，若有则舍弃掉
            # if sample_info["boxcolor"] in [0,1,2,3] and sample_info["boxshape"]==5 and sample_info["toward_orientation"]==1:
            #     sample_info["lightboxcolor_head"]=False
    ...
    def get_gt_labels(self):
        ...
        return color_mask, color_labels_selected, shape_mask, shape_labels_selected, toward_mask, toward_labels_selected
    ...
    def evaluate(self,
        ...
        color_mask, color_labels, shape_mask, shape_labels, toward_mask, toward_labels= self.get_gt_labels()
        ...
        if True:
            self.saveflag+=1
            savename="/disk3/zs1/mmclassification/work_dirs/neightlight/"+str(self.saveflag)+"_result.json"
        ...
        if 'accuracy' in metrics:
            ...
            if isinstance(topk, tuple):
                ...
                eval_results_ = {'color_accuracy': color_acc, 'shape_accuracy': shape_acc, 'toward_accuracy': toward_acc}
