...
class ImbalancedDatasetSampler(Sampler):
    ...
    def __init__(self, dataset, indices: list = None, num_samples: int = None,num_replicas=None, rank=None):
        ...
        df = pd.DataFrame()
        df["color_labels"],df["shape_labels"],df["toward_labels"],df["character_labels"],df["simplelights_labels"]=dataset.get_sampler_labels() 
        df["contact"]=df["color_labels"].apply(str)+df["shape_labels"].apply(str)+df["toward_labels"].apply(str)+df["character_labels"].apply(str)+df["simplelights_labels"]
